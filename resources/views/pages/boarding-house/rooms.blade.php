...
@section('content')
    <div id="Content-Container"
        ...
        <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
            ...
            <p class="font-semibold text-white">Choose Available Room</p>
            <div class="dummy-btn w-12"></div>
        </div>

        <div id="Header" class="relative flex items-center justify-between gap-2 px-5 mt-[18px]">
            <div class="flex w-full rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white shadow-sm">
    ...
    @foreach ($boardingHouse->rooms as $room)
        <label class="relative group cursor-pointer">
            ...
            <div
                class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white transition-all duration-300 group-has-[:checked]:ring-2 group-has-[:checked]:ring-[#8A2BEF] group-has-[:checked]:border-[#8A2BEF]"
                style="border-width: 1px; border-style: solid; border-color: #F1F2F6;"
                onmouseover="this.style.borderColor='#8A2BEF'" 
                onmouseout="if(!this.parentElement.querySelector('input').checked) this.style.borderColor='#F1F2F6'">
    ...
    @endforeach
            ...
            <div id="BottomButton" class="relative flex w-full h-[120px] shrink-0">
                <div class="fixed bottom-[30px] w-full max-w-[640px] px-5 z-50">
                    <button type="submit"
                        class="w-full rounded-full p-[14px_20px] font-bold text-white text-center transition-all active:scale-95 shadow-lg"
